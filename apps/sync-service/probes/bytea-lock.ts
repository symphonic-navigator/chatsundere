...
const url = process.env.TEST_DATABASE_URL ?? 'postgres://chatsundere:dev@localhost:5432/sync_db_test';
const pg = postgres(url, { max: 5 });
const db = drizzle(pg);

await db.execute(sql`DROP TABLE IF EXISTS probe_scratch`);
await db.execute(sql`CREATE TABLE probe_scratch (id uuid primary key default gen_random_uuid(), blob bytea)`);
...
if (identical) for (let i = 0; i < payload.length; i += 4096) if (back[i] !== payload[i]) { identical = false; break; }
console.log('Probe C — bytea 2 MiB round-trip byte-identical:', identical, '(type:', back.constructor.name + ')');
...
console.log('Probe C — FOR UPDATE serialised (2nd acquired after 1st released):', secondAcquiredAt >= firstReleasedAt);
