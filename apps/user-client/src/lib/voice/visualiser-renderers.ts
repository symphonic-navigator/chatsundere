...
export interface RenderOpts {
  /** RGB triplet, 0–255 each, of the active mindspace's accent colour. */
...
export interface BarGeometry {
  /** Whole viewport (chatsundere has no sidebar), viewport-relative. */
  chatview: Bounds;
...
export function drawVisualiserFrame(
  ...
  switch (style) {
    case 'sharp':
      drawSharp(ctx, height, bins, opts, geometry);
      break;
    case 'soft':
      drawSoft(ctx, height, bins, opts, geometry);
      break;
    case 'glow':
      drawGlow(ctx, height, bins, opts, geometry);
      break;
  }
}
...
function drawSharp(
  ...
  for (let i = 0; i < n; i++) {
    const dy = Math.min(bins[i] ?? 0, 1) * maxDy;
    if (dy < 0.5) continue;
