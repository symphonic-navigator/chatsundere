...
const STABLE_SETTINGS_WITH_EXPERT = {
  data: {
    defaultMindspaceId: 'a',
    userTexture: 'cloudy' as const,
    expertModel: 'nano-gpt:some-expert-model',
  },
};
const STABLE_SETTINGS_NO_EXPERT = {
  data: {
    defaultMindspaceId: 'a',
    userTexture: 'cloudy' as const,
    expertModel: null,
...
// Settings mock is set per-test via a module-level variable.
let settingsMockData: typeof STABLE_SETTINGS_WITH_EXPERT | typeof STABLE_SETTINGS_NO_EXPERT =
  STABLE_SETTINGS_WITH_EXPERT;

vi.mock('../../src/data/settings.js', () => ({
  useSettings: () => settingsMockData,
}));
...
describe('PersonaEditor — Ask an expert default toggle', () => {
  it('flips aria-pressed when expert model is configured and toggle is clicked', async () => {
    settingsMockData = STABLE_SETTINGS_WITH_EXPERT;
    ...
    const toggle = await screen.findByRole('button', { name: /ask an expert by default/i });
    // Starts off (false).
  ...
  it('is disabled with a tooltip when no expert model is configured', async () => {
    settingsMockData = STABLE_SETTINGS_NO_EXPERT;
    setup('/app/persona/new');
