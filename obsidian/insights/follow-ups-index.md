...
| Settings button (and any other session-gated header control) disappears after a service-worker refresh while user remains on `/app` | Own small squash AFTER UV-relaxation lands | Discovered 2026-05-21 during UV-relaxation manual QA on Android: SW refresh reloads the page; in-memory `session` is gone (Zustand has no persist), but the URL stays at `/app` and `AppShell` renders without an auth guard. `Gate` only fires as the index route (`/`). Result: Root layout's `{session && <Link to="/settings">}` evaluates false → header shows no Settings, no username. User looks "logged in" but has no way out. Fix shape: introduce a `<ProtectedRoute>` wrapping `/app`, `/settings/*`, `/change-passphrase` so a missing session reroutes to `/` (Gate then redirects to `/login` or `/onboarding` correctly). Pre-existing bug, surfaced by UV-relaxation manual QA only because Chris hit a SW update mid-test. |
