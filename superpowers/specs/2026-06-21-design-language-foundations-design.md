...
- **Overflow-menu accessibility (Laura HARD-2):** menu items — **including disabled ones** — are focusable and announce their disabled reason (a `<button>` with `aria-disabled="true"` — **not** the native `disabled` attribute, which drops it from the tab order — plus an `aria-describedby` pointing at the reason; **never** a non-focusable `<div>`; the click handler no-ops when aria-disabled), so "disabled over hidden" holds for keyboard and screen-reader users, not only sighted-pointer users. This closes, at the source for every new list, the regression class logged in `obsidian/insights/ux-deferrals.md` (2026-06-12, OfferingSlotPicker: disabled entries as non-focusable divs).
