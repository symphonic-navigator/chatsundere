...
28. **~~User has a display Font separate from the Default Mindspace.~~** `Settings.userFont: 'sans' | 'serif' | 'cursive'` (new column). Used wherever the user's own name renders in the UI: Entrance-Hall greeting, user-message blocks in chat, About-Me-Override-Placeholder-Ghosting, Settings-About-Me-Card preview. Rationale: extends Decision 20's "Font = voice" semantic to the user themselves — the user has a voice too, and the wireframe's Settings Mindspace-Picker shows "Chris" in the chosen font, signalling the user-display-font interpretation. Default = serif (matching the prototype's serif greeting heading). When a new persona is created, its `font` field initialises from `Settings.userFont` (the user's own voice is the natural starting point for a persona they create); the user can change it in the editor.
    ...
    **Revoked by Decision 36 (Phase 2.6, 2026-05-24).** Chris's iteration-2 feedback removed the User-Display-Font idea: the user has no TTS-output surface (Font is the visual voice of a *persona* a model speaks through, not of the user themselves), so a per-user font lacked a use case. `Settings.userFont` removed from the schema; new personas default to `serif`.
...
36. **Revoke Decision 28 — Settings drops user-display font.** `Settings.userFont` removed from the schema; new personas default to `serif`. Per-persona font (Decision 20) is unchanged — Font remains the persona's visual voice. The Settings UI no longer surfaces a Font row in the Mindspace picker (per Decision 37 the Font UI lives on the Persona Editor only). Dexie's `'id'`-keyed settings store is schemaless for non-indexed fields, so existing rows still carry an orphaned `userFont` value that's harmlessly ignored — no version bump required.

37. **"Font and Voice" as its own Persona-Editor accordion.** Font moves out of the MindspacePicker into its own accordion section placed between Behavior and Mindspace-Override. TTS / voice configuration lands in the same section in a later block. The MindspacePicker grows a `hideFont?: boolean` prop so it can be reused in both the Persona Editor's Mindspace-Override section (font is in Font-and-Voice now) and in My Settings' Default-Mindspace section (no user-font any more per D36). Updates Decision 33's accordion sequence to: Custom Instructions → Model → Behavior → **Font and Voice** → Mindspace-Override → About-Me-Override.

38. **Dynamic accordion meta — collapsed sections preview their content.** `AccordionCard.meta` widens from `string` to `ReactNode` so callers can compose dynamic previews. Three Persona-Editor sections use this:
    - **Model** shows `<providerDisplayName> · <modelDisplayName>` when both `providerId` and `modelId` are set; otherwise the static `"Pick a provider/model pair"`.
    - **Behavior** shows `"Temperature"` plus an inline `NSFW` badge pill (red border, danger-coloured text) when `adultPersona === true`.
    - **Mindspace — Override** shows `"Using user default"` when `mindspaceId === null`; otherwise `<displayName> · <texture>`.

    Rationale (Chris's iteration-2 feedback): the collapsed accordion was hiding selected-state. Surfacing it as meta lets the user see what's chosen without expanding.

39. **Three-action editor UX in Persona Editor + My Settings.** Both editor surfaces share a new `EditorTopbar` component with three slots:
    - **Back button** (top-left, 40×40): discard semantic. When `isDirty`, prompts via `window.confirm('Discard your unsaved changes?')` before navigating; otherwise navigates immediately. Always navigates to the parent route (`/app/circle` from Persona Editor, `/app` from My Settings).
    - **Title** (centre): static label; no breadcrumb prefix.
    - **"Save & Back"** (top-right pill): persist + navigate combined action. Mirrors the SaveBar's disabled logic.

    The bottom `SaveBar`'s **Save** is now persist-and-stay (was persist+navigate). The `SaveBar.saveLabel` prop lets callers customise the button text ("Save Persona" / "Save Settings"). The latent `bg-bg/95` transparency bug on the SaveBar — leftover from Phase 2.5 — is fixed (`bg-ink/95`). Cancel inside SaveBar discards the draft back to disk (with confirm-on-dirty for My Settings; for the Persona Editor Cancel just navigates to `/app/circle` without re-confirmation since Back already covers the explicit-discard path).

40. **My Settings is draft + Save, not auto-save-on-keystroke.** About Me, Global System Prompt, Default Mindspace (colour + texture) edits write to a local `SettingsDraft` state seeded from `useSettings().data`. The SaveBar (or topbar's Save & Back) diffs the draft against the original and dispatches `updateSettings.mutateAsync(diff)` for the changed fields only. SaveBar is disabled when the draft equals the original (`!isDirty`). **Upstream Providers remain out-of-band** — each provider sheet continues to persist on its own "Test & Save" (Decision 31), independent of the page's draft. Rationale: per-keystroke writes felt invisible and unsafe; an explicit Save matches the persona-editor pattern.

41. **Drop "Room · " breadcrumb prefix from topbars.** Page titles in My Settings, My Circle, and Persona Editor are plain ("My Settings", "My Circle", "New Persona" / `<persona-name>` / "Edit Persona"). Replaces the breadcrumb-style `Room · X` text from Decisions 25 / 27. My Circle's back button is also bumped to the new 40×40 / aria-label="Back" convention used by EditorTopbar, even though Circle itself isn't an editor surface — consistency.

42. **My Account is a sixth Hall room.** The Entrance Hall gains a sixth `RoomTile` ("My Account", icon `⌬`, route `/app/account`). Together with My Settings (Decision 27's fifth room) the Hall has six tiles total: three active (My Circle, My Settings, My Account) and three disabled stubs (My Projects, My History, My Treasury). Extends UX-CONCEPT's "5 rooms" baseline. Rationale: identity / auth / server-linking are distinct concerns from per-persona defaults / providers / prompts; one room per concept matches the rooms model cleaner than overloading My Settings.

43. **One way to reach a thing.** The global topbar's gear-icon shortcut to `/settings` is removed. Account is reachable only via the Entrance Hall's My Account tile. Consistent with the rooms model; eliminates the inconsistency that arose when the gear icon (top-right shortcut) and the tile (in-Hall navigation) both pointed at the same place.

44. **`/app/account` is an accordion-style page with EditorTopbar.** Identical structural pattern to `/app/settings`: a single page with four sections rendered as `AccordionCard`s. Sections: **Account** (username, sign-out, delete-local), **Auth Methods** (passphrase, biometrics, recovery key), **Server Linking** (status + link-to-server hand-off), **About** (version, licence, docs). Each section persists its own state (username rename is inline; biometric flows are transactional; server-link hands off to the wizard) so there is no global draft / SaveBar. The EditorTopbar's Save & Back is hidden via the new `EditorTopbar.hideSaveAndBack` prop. Replaces the deleted tab-based `SettingsLayout` plus `/settings/*` sub-pages.

45. **Server Linking is not a route, it's an accordion section.** The previous `/settings/server-linking` was a redirect to `/onboarding/invitation` — which left the user on the onboarding matrix when they hit Back, the bug Chris caught. The new section renders status + a "Link to server" button that navigates to `/onboarding/invitation?return=/app/account`. The invitation wizard's back-button reads `?return=` (defaulting to `/onboarding`) and navigates there; forward-step navigations preserve the search params so the return-URL survives through all four wizard steps. A shared helper `_return-url.ts` exposes `useReturnUrl()` and `useNavTarget()` to the four step files.

46. **Persona Editor: no bottom SaveBar.** The Phase-2.6 split of Save (bottom, persist + stay) + Save & Back (top, persist + navigate) collapsed to a single Save & Back path: a persona-edit form has no "stay" use-case, and two save-buttons doing similar things created confusion. Discard still happens through Back (with confirm-on-dirty). My Settings keeps its SaveBar — settings have a "stay" use-case (a long About-Me edit doesn't need to leave the page to commit). Asymmetry is fine; the surfaces have different shapes.

47. **AccordionCard `scrollIntoView` on open.** Every `AccordionCard` smooth-scrolls itself into view (`block: 'nearest'`) when it opens — but NOT on the initial mount. An `isInitialRef` guard suppresses the scroll the first time `useEffect` fires so an accordion that mounts with `defaultOpen={true}` doesn't tug the page on render. Polite — only scrolls if needed. Fixes the Upstream Providers case where expanding the accordion would push the provider list below the viewport on small screens.
